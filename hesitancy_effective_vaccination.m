...
n_sim = 1;                                                 % Total number of simulations
...
vac_hesitancy = 0;                                         % Proportion of vaccine hesitant population (i.e., 0.2, 0.4)


rand_vac = 1; % 0: no vaccination; 1: rand vaccination
vacc_all = 1; % 0: age based vaccinaton, 1: vaccinate all
if rand_vac == 1 && vacc_all == 0
    maps_path = sprintf('effective_age/vac_rate_%d/maps', num_daily_vacc);
elseif rand_vac == 1 && vacc_all == 1
    maps_path = sprintf('effective_all/vac_rate_%d/maps', num_daily_vacc);
else
    maps_path = 'no_vacc/maps';
end

sim_len = 420;                 % Simulation length in days     
...
sir = model.sir;
sir_vac = [0, 0.01, 0.03, 0.08, 0.15, 0.6, 2.2, 5.1, 9.3]/100 * 5/100;
...
% load daily tests per thousand people for Italy
load('test_data.mat');

% Randomly select vaccine hesitant particles by age-groups

no_vac_1 = find(model.age == age_groups(1));
no_vac_1 = no_vac_1(randperm(numel(no_vac_1)));
no_vac_1 = no_vac_1(1:ceil(length(no_vac_1)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_2 = find(model.age == age_groups(2));
no_vac_2 = no_vac_2(randperm(numel(no_vac_2)));
no_vac_2 = no_vac_2(1:ceil(length(no_vac_2)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_3 = find(model.age == age_groups(3));
no_vac_3 = no_vac_3(randperm(numel(no_vac_3)));
no_vac_3 = no_vac_3(1:ceil(length(no_vac_3)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_4 = find(model.age == age_groups(4));
no_vac_4 = no_vac_4(randperm(numel(no_vac_4)));
no_vac_4 = no_vac_4(1:ceil(length(no_vac_4)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_5 = find(model.age == age_groups(5));
no_vac_5 = no_vac_5(randperm(numel(no_vac_5)));
no_vac_5 = no_vac_5(1:ceil(length(no_vac_5)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_6 = find(model.age == age_groups(6));
no_vac_6 = no_vac_6(randperm(numel(no_vac_6)));
no_vac_6 = no_vac_6(1:ceil(length(no_vac_6)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_7 = find(model.age == age_groups(7));
no_vac_7 = no_vac_7(randperm(numel(no_vac_7)));
no_vac_7 = no_vac_7(1:ceil(length(no_vac_7)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_8 = find(model.age == age_groups(8));
no_vac_8 = no_vac_8(randperm(numel(no_vac_8)));
no_vac_8 = no_vac_8(1:ceil(length(no_vac_8)*vac_hesitancy));
%--------------------------------------------------------------------------
no_vac_9 = find(model.age == age_groups(9));
no_vac_9 = no_vac_9(randperm(numel(no_vac_9)));
no_vac_9 = no_vac_9(1:ceil(length(no_vac_9)*vac_hesitancy));

vac_hesitated = [no_vac_1; no_vac_2; no_vac_3; no_vac_4; no_vac_5; no_vac_6; no_vac_7; no_vac_8; no_vac_9];

% Store the vaccine hesitant particles id
to_vaccinate = zeros(n,1);

% Assign value of 1 to vaccine hesitant particles (0 - for particles to be
% vaccinated)
to_vaccinate(vac_hesitated) = 1;
...
for i_sim = 1:n_sim
    ...
    for ind = 1:n_its
        ...
        tot_cases(ind) = tot_inf(ind) + tot_imm(ind) + tot_qua(ind) + tot_dead(ind) + tot_iso(ind) + tot_sev_inf(ind);
        ...
        if mod(ind, plt_freq) == 0 || ind == 1
            close all;
            plot_maps(ind, ind_exp, ind_inf, vac_imm, ind_dead, ind_qua_t, ind_iso_t, ind_sev_inf, ...
        ...
        for ind_age=1:length(sir)
            % for not vaccinated particles
            sir_ind = find(e == 2 & age == age_groups(ind_age)  & vac_imm == 0);
            ...
            % for vaccinated particles
            sir_vac_ind = find(e == 2 & age == age_groups(ind_age) & vac_imm == 1);
            %sir_ind = sir_ind(randperm(numel(sir_ind)));
            temp = rand(length(sir_vac_ind),1);
            ts(sir_vac_ind(temp < sir_vac(ind_age) * delta_t)) = ind;
            e(sir_vac_ind(temp < sir_vac(ind_age) * delta_t)) = 7;
        end
        ...
        for ind_age=1:length(sir)
            % not vaccinated particles
            sir_ind = find(e == 6 & age == age_groups(ind_age) & vac_imm == 0);
            %sir_ind = sir_ind(randperm(numel(sir_ind)));
            temp = rand(length(sir_ind),1);
            ts(sir_ind(temp < sir(ind_age) * delta_t)) = ind;
            e(sir_ind(temp < sir(ind_age) * delta_t)) = 7;
            
            % vaccinated particles
            sir_vac_ind = find(e == 6 & age == age_groups(ind_age) & vac_imm == 1);
            %sir_ind = sir_ind(randperm(numel(sir_ind)));
            temp = rand(length(sir_vac_ind),1);
            ts(sir_vac_ind(temp < sir_vac(ind_age) * delta_t)) = ind;
            e(sir_vac_ind(temp < sir_vac(ind_age) * delta_t)) = 7;
        ...
        if rand_vac == 1 && vacc_all == 1
            ...
            vac_ind = find(vac == 0 & to_vaccinate == 0 & (e == 0 | e == 1 | e == 2 | e == 3) & age > age_groups(2));
            ...
            ind_end_imm1 = find(t_vac > t_im1 - delta_t & t_vac < t_im1 + delta_t & vac > 0 & (e == 0  | e == 3));
            temp = rand(length(ind_end_imm1), 1);
            %ind_end_imm1 = (t_vac > t_im1 - delta_t & t_vac < t_im1 + delta_t & vac > 0 & (e == 0  | e == 3) & (temp < 0.52));
            vac_imm(ind_end_imm1(temp < gamma_imm1)) = 1;
            
            ind_end_imm2 = find(t_vac > t_im2 - delta_t & t_vac < t_im2 + delta_t & vac > 0 & (e == 0  | e == 3));
            temp = rand(length(ind_end_imm2), 1);
            %ind_end_imm2 = (t_vac > t_im2 - delta_t & t_vac < t_im2 + delta_t & (e == 0  | e == 3) & vac > 0 & (temp < 0.95));
            vac_imm(ind_end_imm2(temp < gamma_imm2)) = 1;
                        
        elseif rand_vac == 1 && vacc_all == 0
            not_vac_ind = (vac == 0 & to_vaccinate == 0 & (e == 0 | e == 1 | e == 2 | e == 3));
            ...
            elseif sum(not_vac_ind & age == age_groups(7)) > 0
...
%                 vac_groups(1) = vac_groups(1) + length(vac_ind)
            ...
            % immunization after 12 days from the first dose
            ind_end_imm1 = find(t_vac > t_im1 - delta_t & t_vac < t_im1 + delta_t & vac > 0 & (e == 0  | e == 3));
            temp = rand(length(ind_end_imm1), 1);
            vac_imm(ind_end_imm1(temp < gamma_imm1)) = 1; 
            
            % immunization after 7 days from the second dose
            % or after 28 days from the first dose (i.e. interval between
            % two vaccinations is 21 days)
            ind_end_imm2 = find(t_vac > t_im2 - delta_t & t_vac < t_im2 + delta_t & vac > 0 & (e == 0  | e == 3));
            temp = rand(length(ind_end_imm2), 1);
            vac_imm(ind_end_imm2(temp < gamma_imm2)) = 1;
...
save 'output/effect_vacc_2_hes_0_all_new_5.mat'