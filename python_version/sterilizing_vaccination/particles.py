...
class Particles():
    def __init__(self, simulator):
        ...
        self.df = pd.DataFrame(columns=["ind", "days", "exposed", "infected", "qua", "iso", "severe infected", "recovered", "vaccinated", "dead", "susceptible", "total cases"])
    ...
    def update_states(self, i, simulator):
        ...
        self.states = {'exposed': np.where(np.copy(self.epidemic_state)==1, self.epidemic_state, 0), 
                       ...
                       'vaccinated':np.where(np.copy(self.epidemic_state)==10, self.epidemic_state, 0),
        ...
        self.df.loc[i,:] = [i, i*simulator.delta_t, np.count_nonzero(self.states['exposed']), np.count_nonzero(self.states['infected']),
                 ...
                 np.count_nonzero(self.states['severe_inf']), np.count_nonzero(self.states['recovered']), np.count_nonzero(self.states['vaccinated']),
    ...
    def plot(self, simulator,i):
        ...
        self.df.plot(x ='days', y='vaccinated', kind = 'line', color = 'green', ax=ax)
