...
class Particles():
    def __init__(self, simulator):
        ...
        An object of class Particles is characterized by five parameters: 
            ...
            6: True Isolated, 7: Severely Infected, 8: False Isolated,
            9: False Quarantined
            app (array): checks whether the app is installed or not
            test_res (array): test results of the testing mode is stored
        ...
        self.states= {}
        self.df = pd.DataFrame(columns=["nd", "days", "exposed", "infected", "qua", "iso", "severe infected", "recovered", "dead", "susceptible", "total cases"])
        self.test_res = np.zeros((simulator.NUMBER_OF_PARTICLES, 1), dtype=np.int8)
        
        # Initialize array to store the information of particles that have contact tracing app
        self.application = np.random.random((simulator.NUMBER_OF_PARTICLES, 1))
        self.app = np.where((self.application<simulator.TRACING), 1, 0)
        
        # Initialize the cell to store contacts of particles that have contact tracing app
        self.n_id = simulator.NUMBER_OF_PARTICLES
        self.n_time = 2
        self.contact_cell = np.empty((self.n_id, self.n_time), dtype=list)
    ...
    def update_states(self, i, simulator):
        ...
        self.df.loc[i,:] = [i, i*simulator.delta_t, np.count_nonzero(self.states['exposed']), np.count_nonzero(self.states['infected']),
                            ...
                            (np.count_nonzero(self.states['true_iso'])+np.count_nonzero(self.states['false_iso'])),
                             ...
                             np.count_nonzero(self.states['dead']), np.count_nonzero(self.states['susceptible']), 
                             self.states['total_cases']-np.count_nonzero(self.states['exposed'])]
    ...
    def get_new_cases_ids(self, i, simulator):
        ...
        contagious_ids = np.hstack((exposed_id[ind_end_exp], qua_id[ind_end_qua], infected_id, sev_infected_id[ind_end_sev], isolated_id[ind_end_iso])).ravel()
        
        contact_ids = [int(i) for sublist in KDTree(self.x, leaf_size=2, metric="manhattan").query_radius(self.x[contagious_ids], 
